...
function withRequestId(response: Response, reqId: string): Response {
...
function initialConnectionData(request: Request): ConnectionData {
  return {
      ...
      try {
        if (url.pathname === "/ws") {
          const upgraded = serverRef.upgrade(request, {
            data: initialConnectionData(request)
          });
